...
userRouter.post("/login", async (req, res) => {
  try {
    ...
    let userPassword, userId, name, token, role;

    if (isVictimExist) {
      userPassword = isVictimExist.password;
      userId = isVictimExist._id;
      name = isVictimExist.name;
      role = isVictimExist.role;
      token = isVictimExist.generateToken();
    } else if (isVolunteerExist) {
      userPassword = isVolunteerExist.password;
      userId = isVolunteerExist._id;
      name = isVolunteerExist.name;
      role = isVolunteerExist.role;
      token = isVolunteerExist.generateToken();
    } else {
      throw new Error("user not exist");
    }

    const isPasswordValid = isVictimExist
      ? await isVictimExist.checkPassword(password)
      : await isVolunteerExist.checkPassword(password);
